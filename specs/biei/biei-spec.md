...
Production logic belongs in `crates/biei-core`; the executable in
`servers/biei` is intentionally a thin composition root. Submodules are used
only where a boundary has grown enough to benefit from them.

```text
Cargo.toml
specs/biei/biei-spec.md
...
`servers/biei/src/main.rs` runs the production server through the
`biei-core` library entry point.
