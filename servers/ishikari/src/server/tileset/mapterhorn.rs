...
use crate::mapterhorn::{BASE_MAX_ZOOM, MapterhornConfig};
...
#[derive(Clone)]
pub(crate) struct MapterhornResolver {
...
impl MapterhornResolver {
    /// Builds a resolver from configuration already validated at the options
    /// boundary.
    pub(crate) fn new(config: MapterhornConfig) -> Self {
        let (tileset, namespace, maxzoom, negative_ttl) = config.into_parts();
    ...
    fn detail_tileset(&self, z: u8, x: u32, y: u32) -> TilesetId {
        ...
        let y6 = y >> shift;
        self.namespace
            .join_segment(&format!("6-{x6}-{y6}"))
            .expect("Mapterhorn detail archive segment is valid")
    }
}
...
mod tests {
    ...
    async fn base_archive_serves_low_zoom_without_probing() {
        ...
        assert_eq!(
            ...
            Resolved::Base(tileset_id("mapterhorn/planet"))
        );
        assert_eq!(
    ...
    async fn present_detail_is_served_then_cached() {
        ...
        assert_eq!(
            r.resolve(13, 8000, 3000, counting_probe(calls.clone(), true))
                .await
                .unwrap(),
            Resolved::Detail(tileset_id("mapterhorn/6-62-23"))
        );
    ...
    fn matches_only_the_configured_tileset() {
        let r = resolver();
        assert!(r.matches(&tileset_id("mapterhorn/planet")));
        assert!(!r.matches(&tileset_id("mierune/omt")));
        assert!(!r.matches(&tileset_id("mapterhorn/6-0-0")));
    }
}
