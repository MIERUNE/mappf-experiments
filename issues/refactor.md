...
- **Partial prototype landed (Ishikari, 2026-07); item stays open.** A versioned TOML document plus its generated schema exists, but it carries only six tuning settings: listeners, tileset sources, cache budgets, and most routing controls remain flag-only, so it cannot yet express a deployment on its own. Two exclusions are deliberate and constrain any completion: provider URL templates are accepted with fixed query parameters and can therefore embed a token, so admitting them would make the document a secret; and node identity and advertise addresses must differ per replica, so a conventionally shared document must not carry them. Completing this item means resolving those two cases, not simply widening the field list.
- **Selected stack (2026-07, does not authorize work):** `toml` for the document and `schemars` for the schema. TOML is already resolved in this workspace and needs no position on the post-`serde_yaml` fork question; adopt YAML instead only if operators must edit the file as a Kubernetes ConfigMap, and then prefer `serde-saphyr` at its 1.0 line over the unmaintained forks. Hand-write file/environment/flag precedence with `serde` and `clap`; do not adopt a layered-configuration crate.
- **Opinion:** **the strongest of these three for the self-hosted middleware framing, and still gated on a real external operator.** Martin's schema-validated file plus discovery is a genuine operability advantage over flags; the risk here is precedence ambiguity, not parsing.

### 116. Publish a machine-readable description of the public delivery API

- **Trigger:** an external integrator, published client SDK, or documented API commitment.
- **Outcome:** a generated description of the public delivery routes, their parameters, and their error contract, derived from the handlers so it cannot drift from served behavior.
- **Guardrail:** describe public delivery routes only; operational and cluster-internal routes stay undescribed and unexposed. The description is a derived artifact, not the behavioral source of truth — [`../specs/`](../specs/README.md) remains authoritative. Generation must add no request-path cost and must not force a framework migration to obtain annotations.
- **Selected stack (2026-07, does not authorize work):** `utoipa` with its Axum feature plus `utoipa-axum`, so a route and its description are registered together. Rejected `aide` despite its single-schema-library appeal: its stable line still requires the superseded `schemars` 0.9, which would put two schema-library versions in this workspace, and the release that moves to `schemars` 1.x has been in alpha since late 2025. Reconsider only if that release ships stable. Do not add a bundled API-explorer UI.
- **Both 115 and 116:** keep the schema crates optional behind one feature and emit artifacts from a dedicated binary, so neither is linked into the served build and generation stays off the request path. A schema-generating build must enable every feature that contributes configuration or routes, or the published artifact is silently incomplete.
- **Opinion:** **worthwhile once external integrators exist, and only if generated.** Martin annotates its handlers and emits the description from them; a hand-maintained document would drift, which removes most of the value.
