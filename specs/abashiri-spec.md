...
| --- | --- |
| Namespace | Management ownership, delivery identity, and authorization boundary |
...
Every managed style has the canonical logical identity `{namespace}/{style_id}`; a namespace-less style is invalid. Its physical object may use either `styles/{namespace}/{style_id}/style.json` or `styles/{namespace}/{style_id}.json`, selected by deployment configuration. Both layouts address and notify the same logical resource. Namespaced PMTiles archives use `tilesets/{namespace}/{tileset_id}.pmtiles`. A legacy `tilesets/{tileset_id}.pmtiles` archive may remain visible to operators but is not an ordinary user's managed resource.

Abashiri derives style object paths only after validating both identity segments and discovers inventory through bounded listing of the configured state root. Parsed namespace identity is checked against the authenticated grant; a prefix is an identity encoding, not an IAM boundary. The first canonical resource path is `/namespaces/{namespace}/styles/{style_id}`. The experimental `/accounts/{namespace}/styles/{style_id}` spelling remains a compatibility alias while clients migrate.
...
- Any client-supplied namespace is an assertion checked against the authenticated principal, never an authorization source by itself.
...
- Collections use a stable order and bounded storage scan. The first Console pages and filters one namespace-scoped snapshot in the browser; a server-side bounded page size and opaque continuation cursor are required before a single namespace can exceed that scan bound. Clients do not depend on storage-prefix enumeration or cursor internals.
...
- `GET /namespaces/{namespace}/styles/{style_id}` — requires `style.read`, returns exact published bytes and the current opaque strong `ETag`;
- `PUT /namespaces/{namespace}/styles/{style_id}` — requires `style.publish`, one bounded `Idempotency-Key`, `application/json`, and either `If-None-Match: *` for create or the Abashiri ETag in `If-Match` for replace.
- `GET /namespaces` — requires `operations.read`, `style.read`, or `tileset.read`. An operator discovers first-level namespace prefixes through bounded delimiter listing. An ordinary resource reader receives its granted namespaces without enumerating the state store.
- `GET /inventory?namespace={namespace}` — requires permission to read at least one resource kind in the selected namespace. Global `operations.read` may select any discovered namespace. Other principals may select only a granted namespace and receive only the style and tileset kinds authorized by their actions. Unauthorized namespace selection returns `403`; legacy namespace-less tilesets are not part of this Console collection. The response identifies its namespace and whether visibility is `all` or `granted`, and never exposes object-store locations.
...
Abashiri does not join either gossip cluster. After durable completion, the style PUT route posts the same idempotent hint to every configured Biei or Ishikari internal endpoint in parallel. The mutation-key digest is the `hint_id`; receivers fan the hint out locally. Endpoint count and per-request time are bounded, redirects are disabled, and only the fixed internal receiver path is accepted. Hint failure never rolls back or obscures a durable commit. The response reports partial delivery, and replaying the identical mutation with the same idempotency key retries the hint without rewriting style state. Delivery polling remains the correctness path. The canonical style-key validator is shared with the hint envelope, so every accepted publication identity is transportable without a catalog-time validation pass.
...
Sensitive management responses use `Cache-Control: no-store`. The experimental server binds to loopback by default and exposes mutation routes only when auth plus separate state and journal storage are configured.
...
MMPF Console is a dedicated Abashiri management client. It reads management data only through Abashiri and never calls object storage or cluster-internal listeners directly; optional preview links merely navigate the browser to separately configured Biei and Ishikari public routes. Abashiri authenticates the user, authorizes the operation, and aggregates any delivery status exposed through configured service adapters. Service-owned status payloads remain untrusted display data: Console renders them through escaped text or typed components and never injects serialized status as HTML or JavaScript source. A Console failure therefore affects only the user interface, while an Abashiri failure affects management availability but not delivery.

Console discovers the configured browser authentication adapters through the bounded, unauthenticated, non-cacheable `GET /auth/capabilities` bootstrap contract. Object-store bearer authentication remains the baseline for closed or small deployments and credentials are held only in browser memory by default. OIDC sessions and trusted authentication proxies are optional adapters, not platform prerequisites; an unknown method is never treated as bearer authentication.

Hostname and path layout are deployment composition rather than named application profiles. A deployment may omit Abashiri and Console, expose management on a private origin, give Console and Abashiri different origins, colocate them on one management origin, or mount them alongside delivery routes. MMPF Console receives its API base URL from runtime configuration; its static SvelteKit UI base is selected by the deployment build. Abashiri receives its canonical public URL, external API mount path, and exact allowed Console origins from configuration; neither application hard-codes a MIERUNE hostname or infers security-sensitive URLs from untrusted forwarding headers.
  ...
  "credentials": [
    {
      ...
      "namespaces": ["example"],
      "actions": ["operations.read", "style.read", "style.publish", "tileset.read", "tileset.publish"]
    }
  ]
}
```

The object contains no raw bearer credential. `abashiri hash-credential` reads one credential from stdin and emits the digest used by this schema. Actors, namespaces, actions, credential count, and snapshot bytes are bounded and validated before installation. Credentials must be independently generated with at least 256 bits of entropy; the parser's 32-byte minimum is a transport bound and cannot distinguish random material from a weak repeated string. The parser accepts the former `accounts` field as a migration alias, but new registries emit `namespaces`.
...
This registry is not a delivery-auth registry. Its bearer credentials are accepted only by Abashiri and never forwarded to or accepted by Biei or Ishikari. Style and tileset actions are checked together with the target namespace and therefore require at least one namespace grant. `style.read` and `tileset.read` also control namespace-scoped collection visibility. The global `operations.read` action authorizes both the cross-service operational overview and selection of any discovered resource namespace, and may be the sole action on a credential with an empty namespace list; it is not made namespace-scoped by inventing a false resource boundary. The initial transport is one `Authorization: Bearer` header. Query-string management credentials are intentionally unsupported. Human OIDC sessions and external workload-identity adapters remain separate decisions. The object-store adapter accepts only `workload` actors; a registry entry cannot self-identify a bearer credential as a human session.
...
- `GET /namespaces/{namespace}/styles/{style_id}` when style publication is configured
- `PUT /namespaces/{namespace}/styles/{style_id}` when style publication is configured
- `GET` and `PUT /accounts/{namespace}/styles/{style_id}` as a temporary compatibility alias

The health endpoints return `{"status":"ok"}` as JSON. `/whoami` requires a Bearer credential and returns only its bounded actor, namespaces, exact actions, and installed registry revision. During migration it also emits the same grants under the deprecated `accounts` field so Console and Abashiri can roll independently; inventory management identities similarly retain `account_id` beside `namespace`. Every response uses `Cache-Control: no-store`. Without `ABASHIRI_AUTH_ROOT`, `/whoami` is absent and the process remains a health-only server. Style routes are registered only when auth, `ABASHIRI_STATE_ROOT`, and `ABASHIRI_JOURNAL_ROOT` are configured. Remote state and journal URLs sharing one authority are rejected at startup. The auth registry is loaded before listening; partial style configuration is rejected. Readiness still does not continuously prove object storage availability.
