...
pub const MLT_CONTENT_TYPE: &str = "application/vnd.maplibre-tile";
...
impl Directory {
    ...
    pub fn parse(compression: Compression, bytes: Bytes) -> Result<Self> {
        Self::parse_with_limit(compression, bytes, MAX_DECOMPRESSED_BYTES)
    }

    /// Parses a directory while enforcing a caller-selected decompressed-size limit.
    pub fn parse_with_limit(
        compression: Compression,
        bytes: Bytes,
        max_decompressed_bytes: usize,
    ) -> Result<Self> {
        let decompressed = decompress_bytes_with_limit(compression, bytes, max_decompressed_bytes)?;
...
fn read_u64_varint(buf: &mut &[u8]) -> Result<u64> {
    ...
    loop {
        ...
        *buf = &buf[1..];
        ...
        // At shift 63 only a single value bit still fits in a `u64`. A larger
        // payload there (or a further continuation byte) overflows the integer,
        // so reject it instead of silently discarding the high bits and decoding
        // to a wrong offset/tile id.
        if shift == 63 && payload > 1 {
            bail!("invalid PMTiles varint: value exceeds u64");
        }
        value |= payload << shift;
        if byte & 0x80 == 0 {
            return Ok(value);
...
mod tests {
    ...
    #[test]
    fn varint_decodes_max_u64() {
        // Nine 0xFF bytes plus a final 0x01 carry all 64 value bits.
        let mut bytes: &[u8] = &[0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x01];
        assert_eq!(super::read_u64_varint(&mut bytes).unwrap(), u64::MAX);
        assert!(bytes.is_empty());
    }

    #[test]
    fn varint_rejects_overflow_instead_of_truncating() {
        // A 10th byte greater than 1 sets bits above 63; reject it rather than
        // silently dropping them and decoding to a wrong value.
        let mut bytes: &[u8] = &[0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x02];
        let error = super::read_u64_varint(&mut bytes).unwrap_err();
        assert!(error.to_string().contains("exceeds u64"));
    }

    #[test]
    fn varint_rejects_overlong_continuation() {
        // A 10th byte with the continuation bit set demands an 11th byte, which
