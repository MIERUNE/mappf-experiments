...
The `mmpf_mln_resource_*` metrics separate Database cache operations, cache-insertion races, deferred background refreshes, admission wait, single-flight participation, and actual upstream HTTP attempts. Use `--disable-mln-file-sources` only as a diagnostic A/B mode when comparing the Rust cache/loader with MapLibre Native's default leaves.
