...
#[derive(Eq, PartialEq)]
struct ScoredPeerRef<'a> {
    score: u64,
    peer: &'a Peer,
}

impl Ord for ScoredPeerRef<'_> {
...
impl PartialOrd for ScoredPeerRef<'_> {
...
impl HrwRouter {
    ...
    pub fn route_tile(&self, peers: &[Peer], tileset_id: &str, tile_id: u64) -> Vec<ScoredPeer> {
    ...
    fn route_group(&self, peers: &[Peer], key: &str, group_id: u64) -> Vec<ScoredPeer> {
        let mut top_peers =
            BinaryHeap::with_capacity(self.candidate_count.saturating_add(1).min(peers.len()));

        for peer in peers {
            let candidate = ScoredPeerRef {
        ...
        let routed = top_peers
            .into_sorted_vec()
            .into_iter()
            .map(|candidate| ScoredPeer {
