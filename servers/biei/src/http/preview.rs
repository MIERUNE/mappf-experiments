...
/// a renderer/Node.
pub(crate) async fn build_preview_response_for_style<C, Fut>(
...
mod tests {
    ...
    use biei_core::types::ProfilePreparationError;
    ...
    async fn build_preview_response<C, Fut>(
        catalog: &StyleCatalog,
        path: &str,
        check_available: C,
    ) -> IngressResponse
        ...
        Fut: std::future::Future<Output = Result<(), StyleAvailabilityError>>,
    {
        ...
        let style_id = match crate::http::path::resolve_style_id(style_segments) {
            Ok(style_id) => style_id,
            Err(err) => return response_from_ingress_error(err),
        };
        build_preview_response_for_style(catalog, style_id, check_available).await
    }
    ...
    async fn style_check_unavailable(
        ...
        Err(StyleAvailabilityError::Unavailable(
