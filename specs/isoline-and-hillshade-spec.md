...
- `servers/ishikari/src/server/tileset/terrain/mod.rs`: HTTP, cache, admission,
...
- `crates/mmpf-terrain/src/hillshade.rs`: illumination, quantization, and
  vector/raster products
...
The `mmpf-terrain` crate contains service-independent algorithms. Ishikari
continues to own PMTiles resolution, distributed placement, caching, CPU
admission, response negotiation, and HTTP error policy.

Mapterhorn's published source contract is 512px Terrarium tiles. Ishikari
passes that limit explicitly to the decoder and rejects larger images before
materializing their RGB buffers. The reusable crate retains a more permissive
generic safety ceiling because Terrarium itself does not define a tile size;
that generic ceiling is not Ishikari's working-memory assumption.
...
is unreachable or does not expose the typed internal endpoint, the entry node
generates locally rather than failing the public request.
