...
pub(crate) struct DrainController(Arc<AtomicBool>);
...
mod tests {
    ...
    fn only_peer_forwarding_paths_drain() {
        ...
        // Peer-forwarded derived (terrain) generation is expensive; reject on drain.
        assert!(is_drainable_path(
            "/_internal/derived/mapterhorn/planet/contours/10/0/0"
        ));
