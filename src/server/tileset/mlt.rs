...
pub(crate) fn mlt_response_bytes(
    state: &AppState,
    ...
    let bytes = transcoded_mlt(state, tileset_id, tile_id, tile)?;
...
fn transcoded_mlt(
    ...
    if let Some(cached) = state.mlt_cache().get(&key) {
        return Ok(cached);
    }
    ...
    let bytes = Bytes::from(gzip(&mlt)?);
    state.mlt_cache().insert(key, bytes.clone());
    Ok(bytes)
}
